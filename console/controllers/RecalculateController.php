...
class RecalculateController extends AbstractConsoleController
{
    ...
    public function actionProfitBonus(){
        $payments = Payments::find()->andWhere(['>=','pay_date',strtotime("2017-08-01")])->all();
        $schemes = BonusScheme::find()->where(['type'=>BonusScheme::TYPE_PROFIT_PAYMENT])->all();
        foreach($schemes as $item){
            foreach($item->users as $user){
                $bonus = BUserBonus::find()->joinWith('payment')->where(['buser_id'=>$user->id])->andWhere(['>=','pay_date',strtotime("2017-08-01")])->all();
    ...
    public function actionRegenerateActs(){
        $acts = Acts::find()->andWhere(['>=','act_date', '2017-07-01'])->andWhere(['<=','act_date', '2017-07-31'])->all();
        foreach($acts as $act){
