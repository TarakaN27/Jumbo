...
class CustomHelper {
    ...
    */
	...
	public static function num2strEng($num, $code='byn')
	{
		...
		foreach($currStr as $slug=>$val){
			if(stripos($code, $slug) !== false){
				$curr = $val;
				break;
			}
		}
		...
		list($num,$cent) = explode('.',sprintf("%015.2f", floatval($num)));
		
		$num_str = self::num2strEngFn($num);
	...
	public static function num2strEngFn($num = false)
	{
		$num = str_replace(array(',', ' '), '' , trim($num));
