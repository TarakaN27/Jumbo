...
$this->registerJs('
    var
        ...
        B_TYPE_PROFIT_PAYMENT = '.BonusScheme::TYPE_PROFIT_PAYMENT.';
...
<div class="bonus-scheme-form hide">
    ...
    <div class="type2 type3 type6  <?=!in_array($model->type,[BonusScheme::TYPE_UNITS,BonusScheme::TYPE_PAYMENT_RECORDS]) ? '' : 'hide'?>">
    ...
    <div class="pdd-left-10 pdd-right-10 type5 <?=$model->type== BonusScheme::TYPE_PAYMENT_RECORDS ? '' : 'hide'?>">
        <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12 well text-center">
                <button type="button" data-curr-num="0" class="btn btn-info btn-xs addRecordId"><i class="fa fa-plus-square"></i></button>
                <button type="button" class="btn btn-danger btn-xs removeRecordId"><i class="fa fa-minus-square"></i></button>
                <div class="form-group">
                    <div class="row">
                        <div class="col-md-4 col-sm-4 col-xs-12">
    ...
    <div class="pdd-left-10 pdd-right-10 type6 <?=$model->type== BonusScheme::TYPE_PROFIT_PAYMENT ? '' : 'hide'?>">
        <div class="form-group field-bonusscheme-grouping_type  text-center">
            <div class="monthList" data-col="all" data-num="<?=(int)$model->num_month?>">
                <?php
                $countMonth = 0;

                if(isset($arBServices['all']) && !empty($arBServices['all']->month_percent))
                {
                    $countMonth = count($arBServices['all']->month_percent);
                    foreach($arBServices['all']->month_percent as $key => $item) {
                        if(is_int($key)) {
                            $inputEl = Html::textInput('months[all][' . $key . ']', $item, [
                                'id' => 'mid_all_' . $key
                            ]);
                            $labelEl = Html::label($key);
                            echo Html::tag('div', $labelEl . $inputEl, [
                ...
                for ($i = $countMonth+1;$i<=(int)$model->num_month;$i++)
                {
                    $inputEl = Html::textInput('months[all][' . $i . ']', NULL, [
                        'id' => 'mid_all_' . $i
                    ]);
                    $labelEl = Html::label($i);
                    echo Html::tag('div', $labelEl . $inputEl, [
            ...
            <input id ='dub_month' type="checkbox"  <?if(isset($arBServices['all'])&& $arBServices['all']->dublicateLastMonth == 1){?>checked<?}?> value="1" name="months[all][dublicateLastMonth]"><label for="dub_month"><?=Yii::t('app/bonus','Dublicate last month unlim')?></label>
        </div>

        

        <div class="form-group">
            <label class="control-label col-md-3 col-sm-3 col-xs-12"><?=Yii::t('app/bonus','Rates record bonus profit')?></label>
            <div class="col-md-6 col-sm-6 col-xs-12 well text-center">
                <div class="form-group">
                    <input id ='exclude_sale' type="checkbox" value="1" <?if(isset($arRates['exclude_sale'])&&$arRates['exclude_sale'] == 1){?>checked<?}?> name="exclude_sale"><label for="exclude_sale"><?=Yii::t('app/bonus','Exclude sale')?></label></div>
                <button type="button" data-curr-num="0" class="btn btn-info btn-xs addRecordId"><i class="fa fa-plus-square"></i></button>
                <button type="button" class="btn btn-danger btn-xs removeRecordId"><i class="fa fa-minus-square"></i></button>
                <div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-md-4 col-sm-4 col-xs-12">
                            <?=Html::label(Yii::t('app/bonus','Divider'))?>
                        </div>
                    </div>
                </div>
                <section class="recordContainer">
