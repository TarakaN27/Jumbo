...
class ActedPaymentsReportForm extends Model
{
    ...
    {
        $data = ActToPayments::find()->select([
            Acts::tableName() . '.act_date',
            ActToPayments::tableName().'.amount as acted_sum'])
            ->joinWith('act', false)
            ->joinWith('payment')
            ->andWhere(
        ...
        $data->joinWith('payment.calculate');
        $data->joinWith('payment.cuser');
        $data->joinWith('payment.currency');
        //$data->joinWith('cuser.manager');
        $data->joinWith('payment.cuser.requisites');
        if ($this->generateExtendExcel) {
            $data->joinWith('payment.cuser.quantityHour');
            $data->joinWith('payment.cuser.prospects');
            $data->joinWith('payment.cuser.manager manager');
            ...
            $data->joinWith('payment.enrollRequest');
            $data->joinWith('payment.enrollRequest.unitEnroll');
        ...
        $data->joinWith('payment.legal');
        $data->joinWith('payment.service');
        $data->joinWith('payment.calculate.payCond');
        $data->joinWith('payment.sale.buser saleBuser');
        $data->joinWith('payment.payRequest.manager managerBuser');
        $data->joinWith('payment.payRequest.bankDetails bankDetails');
        $data->addSelect($arSelect);
        $data->andFilterWhere([
        ...
        $dates = array_unique(ArrayHelper::getColumn($data, 'pay_date'));
            ...
            */
            ...
            switch ($this->groupType) {
                case self::GROUP_BY_DATE:
                    ...
                    $totalGroupActedSum = $this->totalHelper($totalGroupActedSum, $date, $dt['acted_sum'] * $iCurr);
                    ...
                    if ($dt['pay_cond_id']) {
                        ...
                        $totalGroupActedProfit = $this->totalHelper($totalGroupActedProfit, $date, $dt['profit']*($dt['acted_sum']/ $dt['pay_summ']));
            ...
            if ($dt['pay_cond_id']) {
                ...
                $arResult['iActedProfitTotal'] += $dt['profit']*($dt['acted_sum']/$dt['pay_summ']);
        ...
        $arResult['totalGroupActedProfit'] = $totalGroupActedProfit;
    ...
    public function generateExtendExcelDocument($data, $arCondIDs = [])
    {
        ...
        $objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('D5', 'Общая актированная прибыль')
            ->setCellValue('E5', $data['iActedProfitTotal']);
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('AF9', Yii::t('app/reports', 'Актированный профит'));
        ...
        foreach ($data['data'] as $key => $dt) {
            foreach ($dt as $d) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('AF' . $i, $d['profit'] ? $d['profit']*($d['acted_sum']/$d['pay_summ']): 'N/A');
                $i++;
            }
        }
